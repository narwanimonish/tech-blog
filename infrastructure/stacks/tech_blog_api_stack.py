...
class TechBlogApiStack(Stack):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        api.add_lambda_resource("users", "GET", lambda_stack.users_list.function, authorizer=authorizer)
        api.add_lambda_resource("users/{userId}", "GET", lambda_stack.users_get.function, authorizer=authorizer)
        api.add_lambda_resource("users/{userId}", "PUT", lambda_stack.users_put.function, authorizer=authorizer)
        api.add_lambda_resource("users/{userId}", "DELETE", lambda_stack.users_delete.function, authorizer=authorizer)
        api.add_lambda_resource("posts", "GET", lambda_stack.posts_list.function, authorizer=authorizer)
        api.add_lambda_resource("posts", "POST", lambda_stack.posts_post.function, authorizer=authorizer)
