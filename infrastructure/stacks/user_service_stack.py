...
class UserServiceStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, config: DevConfig|ProdConfig, api_gw: apigw.IRestApi, **kwargs):
        super().__init__(scope, construct_id, **kwargs)

        # 1. Database
        users_table_name = f"{config.APP_NAME}_users"
        users_table = DynamoDBTable(self, id=users_table_name, table_name=users_table_name, partition_key_name="userId")
        ...
        for method in methods:
            ...
            handler = LambdaFunction(
                self, id=function_name, function_name=function_name,
