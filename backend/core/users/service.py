...
from core.users.keys import (
    USERS_BY_CREATION_TIME_INDEX,
    USERS_LIST_PK,
    USERS_LIST_PK_VALUE,
)
...
class UsersService:
    ...
    def _with_list_index_fields(data: dict) -> dict:
        ...
        data[USERS_LIST_PK] = USERS_LIST_PK_VALUE
    ...
    def has_any_users(self) -> bool:
        ...
        page = dynamodb_util.query_page(
            self._table,
            index_name=USERS_BY_CREATION_TIME_INDEX,
            partition_key_name=USERS_LIST_PK,
            partition_key_value=USERS_LIST_PK_VALUE,
            limit=1,
    ...
    def upsert_user(self, user_id: str, data: dict) -> dict:
        ...
        if existing:
            ...
            self._with_list_index_fields(merged)
        ...
        item = self._with_list_index_fields({**data, "userId": user_id, "role": role})
    ...
    def list_users(self, *, limit: int = pagination_util.DEFAULT_PAGE_SIZE, start_key: dict | None = None) -> dict:
        """List one page of users (newest first) via GSI UsersListByCreationTime."""
